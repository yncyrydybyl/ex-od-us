---
name: Linux Surface
description: Linux Kernel for Surface Devices
repo: "https://github.com/linux-surface/linux-surface"
platform: github
categories: [Development]
status: Active
issues: [6939]
updated: "2026-04-03T09:32:49Z"
matrix_links:
  - target: "#linux-surface-development:matrix.org"
    kind: room
    via: matrix.to
    source: anchor
    quality: 7
  - target: "#linux-surface-support:matrix.org"
    kind: room
    via: matrix.to
    source: anchor
    quality: 7
  - target: "#linux-surface:matrix.org"
    kind: room
    via: matrix.to
    source: anchor
    quality: 7
matrix_rooms: [https://matrix.to/#/#linux-surface-development:matrix.org, https://matrix.to/#/#linux-surface:matrix.org, https://matrix.to/#/#linux-surface-support:matrix.org]
exodus_score: 3
last_scanned: "2026-04-06T20:35:19Z"
---

Linux Kernel for Surface Devices

## Stats

- Stars: 7057
- Forks: 310
- Language: Shell
- Topics: linux, linux-surface, surface-devices
