---
name: "Rgfw"
description: "A cross platform lightweight single-header simple-to-use window abstraction library for creating graphical programs or libraries."
repo: "https://github.com/ColleagueRiley/RGFW"
platform: github
categories: [Development]
status: "Active"
exodus_score: 3
last_scanned: "2026-04-06T22:14:33Z"
matrix_links:
  - target: "#rsgl-is-sili:matrix.org"
    kind: room
    via: matrix.to
    source: anchor
    quality: 7
matrix_rooms:
  - "https://matrix.to/#/#rsgl-is-sili:matrix.org"
issues: []
updated: "2026-04-02T23:02:49Z"
---

A cross platform lightweight single-header simple-to-use window abstraction library for creating graphical programs or libraries.

## Stats

- Stars: 1735
- Forks: 76
- Language: C
- Topics: c99, cocoa, cross-platform, graphics, library, lightweight, macos, opengl, pure-c, silicon, single-header, unix, webasm, winapi, windowing, x11, xlib
