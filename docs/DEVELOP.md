...
3. **Enrichment**: `scripts/enrich-projects.py` fetches each project's README, scores Matrix presence, extracts room links, checks liveness
...
│   ├── enrich-projects.py       # Fetch READMEs, score Matrix presence
...
│   ├── scan-readmes.sh          # Legacy scanner (replaced by enrich-projects.py)
│   └── notify-changes.sh        # Post scanner results as issue comments
├── data/
│   ├── discovered-slugs.txt     # All 2200+ repo slugs from Sourcegraph
│   ├── readme-cache.json        # ETag + SHA cache for enricher
│   └── score-history.jsonl      # Score snapshots over time
├── tests/
│   ├── test_build_projects.py   # Frontmatter parser tests
│   ├── test_enrich_projects.py  # Scoring, validation, roundtrip tests
...
```bash
# Score all projects (fetches READMEs from GitHub)
python3 scripts/enrich-projects.py --force --skip-matrixrooms

# Score one specific project
python3 scripts/enrich-projects.py --project bspwm --force

# Dry run (shows what would change)
python3 scripts/enrich-projects.py --dry-run
```

### Run the full pipeline

```bash
bash scripts/refresh-all.sh              # normal (uses cache)
bash scripts/refresh-all.sh --force      # ignore cache
